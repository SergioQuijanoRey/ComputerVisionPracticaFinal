...
Entonces, ¿por qué trabajamos con esta base de datos? En un primer momento, exploramos bases de datos más adecuadas para este problema, que no tuviesen que ver con el reconocimiento de caras o de firmas manuales. Por ejemplo, con una base de datos de figuras \emph{Lego} \cite{lego_database:online}. Sin embargo, esta base de datos era demasiado compleja, tal y como nos indicó el profesor de teoría de la asignatura (nos indicó el problema de que muchas figuras salían en posturas demasiado diferentes, como por ejemplo, de cara a la cámara y de espaldas). Nos encontramos con problemas parecidos buscando bases de datos más adecuadas a las características fundamentales presentadas previamente.
...
\begin{figure}[H]
    ...
    \caption{Curva de aprendizaje para el entrenamiento del modelo usando triples aleatorios. En azul, la función de pérdida en entrenamiento. En naranja, la función de pérdida en validación. En el eje $x$ mostramos las veces que se calculan las métricas, que corresponden con 10 \emph{batches}}
\end{figure}
...
\subsection{Adaptación a una tarea de clasificación} \label{triples_aleatorios_adaptacion_clasificacion:seccion}
...
Por otra parte, si ahora extraemos algunas métricas como son el \emph{accuracy} del clasificador, el Área bajo la curva ROC o la métrica Silohuette sobre el modelo de clasificación KNN obtenemos lo siguiente:
...
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.5 \textwidth]{triples_online_curva_aprendizaje}
    \caption{Curva de aprendizaje para el entrenamiento del modelo usando triples difíciles \emph{online}. En azul, la función de pérdida en entrenamiento. En naranja, la función de pérdida en validación. En el eje $x$ mostramos las veces que se calculan las métricas, que corresponden con 10 \emph{batches}}
\end{figure}

En primer lugar, vemos que no hemos caído en problemas de sobre-aprendizaje. En segundo lugar, vemos que tras los 10 primeros \emph{batches}, la red no mejora apenas. Por tanto, podríamos haber entrenado durante muchas menos épocas, obteniendo los mismos resultados.

En el conjunto de \emph{test} obtenemos un valor de la función de pérdida de $0.00102$. Nos aproximamos prácticamente al límite de la función de pérdida debido al margen escogido para dicha función de pérdida. Por tanto, en este sentido estamos satisfechos con los resultados obtenidos.

\subsubsection{Resultados de clasificación}

De nuevo, realizamos una adaptación de la red a una tarea de clasificación. Este proceso ya fue explicado en \customcite{triples_aleatorios_adaptacion_clasificacion:seccion}, y por tanto no lo volvemos a explicar.

Comenzamos mostrando el gráfico del \emph{embedding} obtenido en la siguiente figura:

\begin{figure}[H]
    \centering
    \includegraphics[width = 0.5 \textwidth]{triples_online_embedding}
    \caption{Gráfica del \emph{embedding} calculado por la red sobre el conjunto de entrenamiento. Los colores muestran la clase asociada a cada uno de los puntos que mostramos}
    \label{triples_online_grafica_embedding:figure}
\end{figure}

Se pueden observar que obtenemos \emph{clusters} coherentes, pero que se solapan entre sí. Por tanto, esperamos obtener una métrica \emph{Silohuette} muy mala, cercana a cero.
...
\begin{center}
    \begin{tabular}{|c|c|c|c|}
        ...
        Dataset & Accuracy & Área bajo la curva ROC & Silhouette \\
        \hline
        \emph{train} & 0.266 & 0.665 & -0.095 \\
        \emph{test} & 0.266 & 0.663 & -0.099 \\
        \hline
    \end{tabular}
\end{center}
    \caption{Métricas adicionales del modelo entrenado usando triples difíciles \emph{online}}
\end{table}

Podemos observar que tampoco se aprecia sobre-aprendizaje en la adaptación de la red a una tarea de clasificación. Sin embargo, los resultados siguen siendo muy malos. Era de esperar el mal valor de la métrica de \emph{Silohuette}, por lo que hemos visto en la gráfica del \emph{embedding}. Sin embargo, esperábamos obtener un mejor valor de \emph{accuracy}. Teniendo en cuenta los resultados mostrados en \customcite{triples_aleatorios_resultados_clasificacion:tabla}, estamos duplicando el \emph{accuracy} pero manteniendo un valor muy bajo.

\subsection{Conclusiones} \label{triples_online_resultados:seccion}

Como hemos podido observar en este experimento, los resultados han mejorado algo en comparación con el modelo entrenado con Triples aleatorios, (estamos duplicando el \emph{accuracy} del modelo) pero siguen siendo muy pobres y claramente no se podría tomar como una solución al problema de clasificación multiclase planteado.

Un claro problema, que vemos en \customcite{triples_online_grafica_embedding:figure}, es que obtenemos \emph{clusters} coherentes (distancia \emph{intracluster} pequeña), pero muy solapadas entre sí. Como comentaremos más adelante en \customcite{conclusiones_finales:seccion}, pensamos que modificando el valor del margen conforme avanzan las épocas de entrenamiento.
